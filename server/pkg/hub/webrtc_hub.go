...
func NewWebRTCHub(iceServers []string, natIPs []string) (*WebRTCHub, error) {
	...
	// Register H.264 constrained-baseline profile (42e01f)
	if err := mediaEngine.RegisterCodec(webrtc.RTPCodecParameters{
		...
		return nil, fmt.Errorf("failed to register h264 constrained-baseline codec: %w", err)
	}
...
func (b *WebRTCHub) VideoTrack() *webrtc.TrackLocalStaticRTP {
	return b.videoTrack
}

func (b *WebRTCHub) AudioTrack() *webrtc.TrackLocalStaticRTP {
	return b.audioTrack
}

func (b *WebRTCHub) HandleOffer(offer webrtc.SessionDescription) (*webrtc.SessionDescription, error) {
...
func (b *WebRTCHub) WriteVideoRTP(pkt *rtp.Packet) error {
	pkt.Header.PayloadType = 96
	return b.videoTrack.WriteRTP(pkt)
}

func (b *WebRTCHub) WriteAudioRTP(pkt *rtp.Packet) error {
	pkt.Header.PayloadType = 111
	return b.audioTrack.WriteRTP(pkt)
}

func (b *WebRTCHub) Close() {
	b.mu.Lock()
	defer b.mu.Unlock()
