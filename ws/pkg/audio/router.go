	...
	for _, sec := range sections {
		...
		for _, line := range lines {
			if strings.Contains(line, "application.name =") ||
				strings.Contains(line, "application.process.binary =") ||
				strings.Contains(line, "application.icon_name =") ||
				strings.Contains(line, "media.name =") ||
				strings.Contains(line, "module-stream-restore.id =") {
				parts := strings.Split(line, "=")
				if len(parts) > 1 {
					props = append(props, strings.Trim(strings.TrimSpace(parts[1]), "\""))
				}
			}
		}
		...
		if len(props) > 0 {
			...
			if isBlacklisted {
				...
				// Non-blacklisted app -> Route into stream_sink
